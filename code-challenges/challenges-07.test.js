...
const howMuchPencil = (str) => {
  ...
  for(let i = 0 ; i < str.length+1 ; i++){
    const spil= str.slice(i,str.length);
...
const listFoods = (recipe) => {
  ...
  for(let i = 0 ; i < recipe.ingredients.length ; i++){
    ...
    const start = recipe.ingredients[i].indexOf(" ")
    const firstSlice = recipe.ingredients[i].slice(start+1,recipe.ingredients[i].length)

    const startTwo = firstSlice.indexOf(' ')

    const lastSlice =firstSlice.slice(startTwo+1)
