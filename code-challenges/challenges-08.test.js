...
const isNum = (input) => {
  ...
  const letter = /[0-9]/g;
  const result = input.toString().match(letter);
...
const containsWorld = (input) => {
  // Solution code here...
  const letter = /world/g;
  const result = input.match(letter);
...
const isCapitalized = (str) => {
  ...
  for(let i = 0 ; i < splitArr.length ; i++){
    result = splitArr[i].match(letter);
    result !== null  && newArr.push(splitArr[i].replace(").","").replace(",",""))
  }
...
const citiesAtoJ = (arr) => {
  ...
  for(let i = 0 ; i < arr.length ; i++){
    const result= arr[i].match(letter);
    result !== null  && newArr.push(arr[i])
  }
  return newArr
};
...
const matchMonth = (input) => {
  // Solution code here...
  const letter = /\b(?:October|Oct|october|oct)\b/g
