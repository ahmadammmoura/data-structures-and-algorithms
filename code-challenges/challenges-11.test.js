...
const totalSum = (input) => {
  ...
  input.forEach( num => Array.isArray(num) ? sum += totalSum(num) : sum += num)
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  const filterd = input.map(array =>{
    const filterNested = array.filter(number =>{
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  const filterd = data.filter(person =>{
...
let findShortest = (data) => {
  ...
  const arr =[]
  // Solution code here...
  const shortest = data.map(person =>{
    const result = person.name.match(/([A-z])/g)
  ...
  return shortest[indexOfMaxValue]
